...
if ($lFormSubmitted){
    try {
        ...
        if ($lProtectAgainstMethodTampering) {
            ...
            $lUsername = trim($_REQUEST["username"]);
            $lPassword = $_REQUEST["password"];
            $lConfirmedPassword = $_REQUEST["confirm_password"];
            $lUserSignature = $_REQUEST["my_signature"];
            $lFirstName = trim($_REQUEST["firstname"]);
            $lLastName = trim($_REQUEST["lastname"]);
            $lPostedCSRFToken = $_REQUEST['csrf-token'];
        }

        // Escapar toda salida para mitigar XSS
        $lUsernameText = htmlspecialchars($lUsername, ENT_QUOTES, 'UTF-8');
        $lFirstNameText = htmlspecialchars($lFirstName, ENT_QUOTES, 'UTF-8');
        $lLastNameText = htmlspecialchars($lLastName, ENT_QUOTES, 'UTF-8');
        $lUserSignatureText = htmlspecialchars($lUserSignature, ENT_QUOTES, 'UTF-8');
...
if (!preg_match($pattern, $lPassword)) {
    $lValidationFailed = true;
    echo '<h2 class="error-message">
        Password too weak. It must be at least 8 characters long and include:
        <ul>
        ...
        if (!$lValidationFailed){
            // -----------------------------
            // Prepared statement para SQL seguro
            // -----------------------------
            $stmt = $mysqli->prepare(
                "INSERT INTO accounts (username, password, firstname, lastname, mysignature, client_id, client_secret) 
                 VALUES (?, ?, ?, ?, ?, ?, ?)"
            );
            ...
            if ($stmt) {
                ...
                $stmt->execute();
                $lRowsAffected = $stmt->affected_rows;
                $stmt->close();
...
function onSubmitOfForm(theForm){
    try{
        if(lValidateInput == "TRUE"){
            ...
            if (theForm.username.value.length > 15 || theForm.password.value.length > 15){
...
<form action="index.php?page=register.php" method="post" enctype="application/x-www-form-urlencoded"
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                <input type="text" name="username" size="15" autofocus
        ...
        <tr>
            ...
            <td>
                <textarea rows="3" cols="50" name="my_signature"
                    <?php if ($lEnableHTMLControls) {echo $lHTMLControls;} ?>></textarea>
            </td>
        </tr>
        <tr><td>&nbsp;</td></tr>
        <tr>
